...
impl StegHideCommandBuilder {
    ...
    // Builder functions
    /// `with_command` is required, returns the Builder for further costumizing
    ...
    /// `with_passphrase` is required, returns the Builder for further costumizing
    pub fn with_passphrase(mut self, passphrase: String) -> StegHideCommandBuilder {
    ...
    //    self.{} = {};
    //    }'|sed 's/ARG-REMOVE//g'
    pub fn with_embedfile(&mut self, embedfile: String) {
        ...
        if self.command_mode != Some(super::CommandMode::Embed) { // Arguments.cc:193
            ...
            self.cli_exit_clap_invalidvalue("the argument 'embedfile' can only be used with the 'embed' command mode".to_string());
        }
        self.embedfile = Some(embedfile);
    }
    pub fn with_extractfile(&mut self, extractfile: String) {
        ...
        if self.command_mode != Some(super::CommandMode::Extract) { // Arguments.cc:224
            ...
            self.cli_exit_clap_invalidvalue("the argument 'extractfile' can only be used with the 'extract' command mode".to_string());
        }
        self.extractfile = Some(extractfile);
    }
    pub fn with_coverfile(&mut self, coverfile: String) {
        ...
        if self.command_mode != Some(super::CommandMode::Embed) { // Arguments.cc:255
            ...
            self.cli_exit_clap_invalidvalue("the argument 'coverfile' can only be used with the 'embed' command mode".to_string());
        }
        self.coverfile = Some(coverfile);
    }
    pub fn with_stegofile(&mut self, stegofile: String) {
        ...
        if self.command_mode != Some(super::CommandMode::Embed) && self.command_mode != Some(super::CommandMode::Extract) { // Arguments.cc:286
            ...
            self.cli_exit_clap_invalidvalue("the argument 'stegofile' can only be used with the 'embed' or 'extract' command mode".to_string());
        }
        self.stegofile = Some(stegofile);
    }
    pub fn with_infofile(&mut self, infofile: String) {
        trace!("with_infofile: {}", infofile);
        self.infofile = Some(infofile);
    }
    pub fn with_nochecksum(&mut self) {
        ...
        if self.command_mode != Some(super::CommandMode::Embed) { // Arguments.cc:339
            ...
            self.cli_exit_clap_invalidvalue("the argument 'nochecksum' can only be used with the 'embed' command mode".to_string());
        }
        self.nochecksum = true;
    }
    pub fn with_compress(&mut self, compress: String) {
        ...
        if self.command_mode != Some(super::CommandMode::Embed) { // Arguments.cc:365
            ...
            self.cli_exit_clap_invalidvalue("the argument 'compress' can only be used with the 'embed' command mode".to_string());
        }
        if self.compression_level.is_none(){
            ...
            self.cli_exit_clap_invalidvalue("the arguments 'compress' and 'dontcompress' are conflictive".to_string());
        }
    }
    pub fn with_dontcompress(&mut self) {
        ...
        if self.command_mode != Some(super::CommandMode::Embed) { // Arguments.cc:388
            ...
            self.cli_exit_clap_invalidvalue("the argument 'dontembedname' can only be used with the 'embed' command mode".to_string());
        }
        if self.compression_level.is_none(){
    ...
    pub fn with_dontembedname(&mut self) {
        trace!("with_dontembedname");
        if self.command_mode != Some(super::CommandMode::Embed) { // Arguments.cc:388
            error!("dontcompress provided for non embed operation");
            self.cli_exit_clap_invalidvalue("the argument 'dontcompress' can only be used with the 'embed' command mode".to_string());
        }
        self.embed_name = false;
    }
    pub fn with_radius(&mut self, radius: String) {
        trace!("with_radius: {}", radius);
        self.radius = radius.parse::<u64>().unwrap();
    }
    pub fn with_goal(&mut self, goal: String) {
        trace!("with_goal: {}", goal);
        self.goal = goal.parse::<f64>().unwrap();
    }
    pub fn with_marker(&mut self, marker: String) {
        trace!("with_marker: {}", marker);
        self.marker = marker;
    }
    pub fn with_force(&mut self) {
        trace!("with_force");
        self.force = true;
    }
    pub fn set_debug(&mut self, mode: super::DebugMode) {
        match self.debug_mode {
            None => self.debug_mode = Some(mode),
            Some(_) => {
                self.cli_exit_clap_invalidvalue("debug can only have one mode".to_string());
            }
        };
    }
    pub fn with_debug_printgraph(&mut self) {
        trace!("with_debug_printgraph");
        self.set_debug(super::DebugMode::PrintGraph);
    }
    pub fn with_debug_printgmlgraph(&mut self, printgmlgraph: String) {
        trace!("with_debug_printgmlgraph: {}", printgmlgraph);
        self.set_debug(super::DebugMode::PrintGmlGraph);
    }
    pub fn with_debug_printgmlvertex(&mut self, printgmlvertex: String) {
    ...
    pub fn with_debuglevel(&mut self, debuglevel: String) {
        trace!("with_debuglevel: {}", debuglevel);
        self.set_debug(super::DebugMode::DebugLevel(debuglevel.parse::<u64>().unwrap()));
    }
    pub fn with_debug_printstats(&mut self) {
        trace!("with_debug_printstatsq");
        self.set_debug(super::DebugMode::PrintStats);
    }
    pub fn with_check(&mut self) {
        trace!("with_check");
        self.set_debug(super::DebugMode::Check);
    }
    pub fn with_encryption(&mut self, encryption: String) {
        trace!("with_encryption: {}", encryption);
        if self.command_mode != Some(super::CommandMode::Embed) { // Arguments.cc:193
            error!("encryption provided for non embed operation");
            self.cli_exit_clap_invalidvalue("the argument 'encryption' can only be used with the 'embed' command mode".to_string());
        }
        self.cli_exit_clap_invalidvalue("--encryption is not yet supported".to_string());
    }
...
pub fn parse_arguments() -> Result<super::StegHideSetup,String> {
    ...
    let verbosity = matches.value_of("verbosity").map(|v| {
    ...
    let command_mode = matches.value_of("MODE").unwrap();
    let mut builder = StegHideCommandBuilder::new()
        .with_command(command_mode.to_string())
        .with_passphrase(passphrase.to_string());
    // This should be some sort of macro... Generated with:
    // grep long src/cli.yml|cut -d: -f2|xargs -i{} echo -e '    if matches.is_present("{}"){\n        let {} = matches.value_of("{}").unwrap();\n        builder.with_{}({}.to_string());\n    }'
    ...
    if matches.is_present("nochecksum"){
        let nochecksum = matches.value_of("nochecksum").unwrap();
...
mod tests {
    ...
    fn it_validates_invalid_stdin_both_cover_embed() {
        let mut builder = StegHideCommandBuilder::new()
            ...
            .with_passphrase("aoeu".to_string());
        builder.with_coverfile("-".to_string());
        builder.with_embedfile("-".to_string());
    ...
    fn it_checks_passphrase_argument_needed_on_embed() {
        let mut cover_none = StegHideCommandBuilder::new()
            .with_command("embed".to_string());
        cover_none.with_embedfile("a".to_string());
        assert_eq!(
